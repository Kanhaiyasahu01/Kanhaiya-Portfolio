  ...
  return (
    <section className="w-full h-[90vh] flex justify-center items-center relative mt-8 sm:mt-4 md:mt-0 overflow-hidden">
      <div className="absolute inset-0 overflow-hidden -z-5">
        ...
        <div className={`absolute top-16 -left-20 w-96 h-96 rounded-full bg-gradient-to-br from-blue-500/30 to-blue-700/20 blur-3xl transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}></div>
        
        {/* Smaller purple-blue gradient blob */}
        <div className={`absolute bottom-10 -left-20 w-80 h-80 rounded-full bg-gradient-to-tr from-richblue-600/20 to-blue-400/10 blur-3xl transition-opacity duration-1000 delay-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}></div>
        
        {/* Medium size blue gradient in center-right */}
        <div className={`absolute top-1/3 right-1/4 w-72 h-72 rounded-full bg-gradient-to-bl from-blue-300/20 to-indigo-500/10 blur-3xl transition-opacity duration-1000 delay-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}></div>
      </div>
      {/* Container */}
      <div className="flex flex-col md:flex-row items-center justify-between sm:px-2 w-10/12 mx-auto pb-[5vh] gap-6 sm:gap-8">
        {/* Left Text Section */}
        <div className={`text-white text-center md:text-left space-y-4 sm:space-y-5 transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <div className="space-x-2 sm:space-x-3 md:space-x-4 mt-3 sm:mt-4 md:mt-5 flex justify-center md:justify-start">
            <Link to={'/projects'} className="border z-10 border-blue-500 text-white px-3 sm:px-4 md:px-5 py-1.5 sm:py-2 rounded-lg shadow-lg hover:bg-blue-500 hover:text-white transition-all text-xs sm:text-sm md:text-base">
              See my work
            </Link>
          </div>
        </div>
        ...
        {/* Right Image Section */}
        <div className={`relative flex-shrink-0 mt-6 sm:mt-8 md:mt-0 transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <div className={`absolute top-4 sm:top-6 md:top-10 -left-4 sm:-left-6 md:-left-10 w-10 sm:w-12 md:w-16 h-10 sm:h-12 md:h-16 bg-blue-500 rounded-full animate-bounce shadow-lg transition-opacity duration-1000 delay-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}></div>
        </div>
      </div>

      {/* Social Media Icons */}
      <div className={`absolute top-1/2 right-2 sm:right-4 md:right-10 transform -translate-y-1/2 space-y-2 sm:space-y-3 md:space-y-4 text-white hidden sm:flex flex-col transition-all duration-1000 delay-500 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
        <a
